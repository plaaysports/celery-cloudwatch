...
class PrintCamera(Camera):
    ...
    def on_shutter(self, state):
        ...
        print('Time to Start')
        total = Stats()
        for method_name, stats in list(state.time_to_start.items()):
        ...
        total = Stats()
        for method_name, stats in list(state.time_to_process.items()):
            print("%s avg:%.2fs, max:%.2fs, min: %.2fs" % (method_name, stats.average() or -1.0, stats.maximum or -1.0, stats.minimum or -1.0))
            total += stats
        ...
        print('')
        print('Event Totals')
        methods = set(list(state.task_event_sent.keys()) + list(state.task_event_started.keys()) +
                      list(state.task_event_succeeded.keys()) + list(state.task_event_failed.keys()))
